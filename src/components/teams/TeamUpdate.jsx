...
import TeamFormShimmer from './TeamFormShimmer'
...
const TeamUpdate = () => {
    ...
    const handleTeamUpdate =  async (e) => {
        ...
        try {
            ...
            const response = await fetch('http://localhost:8000/api/team/update/'+teamId, {
                method: 'POST',
                credentials: 'include',
    ...
    useEffect(() => {
        const getTeamData  =  async () => {
            try{
                ...
                const token = getCookie('token');
                const response = await fetch('http://localhost:8000/api/team/edit/'+teamId, {
                    method: 'GET',
                    headers: {
                ...
                if (json?.status === 'success') {
                    ...
                    setAbout(json.data.short_desc);
                    setPriview(json.data.logo);
                }
            }catch(err){
    ...
    >
        <div className="flex justify-center align-middle m-auto gap-4 p-10">
            <div className="w-2/4 bg-slate-200 p-4">
                <legend className="fieldset-legend align-middle justify-center">Update Team</legend>
